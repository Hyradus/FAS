...
################################# FOURTH BLOCK
#statistic_df = gpd.read_file('/media/hyradus/I-DATS/Working/Erica-FFC/Atlas/Processing_original/faults-atlasv03_repr_densified_0_complete_statistics.shp')
...
fname = processing_folder+'/atlas_MaxDisp_Length_normalized.eps'
...
# ax.set_title("y=$%.6fx^{%.6f}$ \n$R^{2}$: %.6f"%(popt_log[0],popt_log[1],r_squared))
ax.set_title('atlas MaxD/L')
fname = processing_folder+'/atlas_maxDLength_ratio.eps'
plt.savefig(fname, format='eps')
...
# ax.set_xticks(np.arange(0,50, step=1))
# ax.set_title("y=$%.6fx^{%.6f}$ \n$R^{2}$: %.6f"%(popt_log[0],popt_log[1],r_squared))
ax.set_title('atlas Max Displacement/Max Aperture')
fname = processing_folder+'/atlas_maxD-maxA log.eps'
plt.savefig(fname, format='eps')
...
# ax.set_xticks(np.arange(0,50, step=1))
# ax.set_title("y=$%.6fx^{%.6f}$ \n$R^{2}$: %.6f"%(popt_log[0],popt_log[1],r_squared))
ax.set_title('atlas Length/Aperture')
fname = processing_folder+'/atlas_Ape-Len log.eps'
plt.savefig(fname, format='eps')
plt.savefig(fname.split('.eps')[0]+'.png', format='png')
...
for plt_chnk in plot_chunks:
    ...
    id1 = str(plt_chnk[0])
    id2=str(plt_chnk[-1])
    fname = processing_folder+'/Atlas_Displacement_along_faults_'+"{0}-{1}".format(id1, id2)+'normalized.png'
    
    plt.savefig(fname, dpi=150)
    fname = processing_folder+'/Atlas_Displacement_along_faults_'+"{0}-{1}".format(id1, id2)+'normalized.pdf'
