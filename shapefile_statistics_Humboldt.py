...
def transect_reshaper(dataframe_name):
    ...
    for idx in id_list:    
            ...
            for sub_idx in sub_idxs:
                ...
                #        corr_len = curr_len/1.3
                elif  15000 < fault_len <25000:
                        corr_len = curr_len/2
                # elif  20000 < fault_len <25000:
                #        corr_len = curr_len/1.6
                # elif  15000 < fault_len <20000:
                #        corr_len = curr_len/1.8
                # elif  10000 < fault_len <15000:
                #        corr_len = curr_len/2.1
                elif  1000 < fault_len <15000:
                        corr_len = curr_len/2
                # # elif  20000 < fault_len <26000:
                # #         corr_len = curr_len/1.9
                # # elif  18000 < fault_len <20000:
                # #        corr_len = curr_len/2.4
                # # elif  12000 < fault_len <18000:
                # #        corr_len = curr_len/2.6
                else:
                    corr_len = curr_len/10
                # elif 17000 <= curr_len < 26000:
                #     fault_len = curr_len/20
                # elif 17000 <=  curr_len < 21000:
                #     corr_len = curr_len/20
                # elif 16000 <= fault_len <17000:
                # #      corr_len = curr_len/
                # elif 12000 <= fault_len <17000:
                #     corr_len = curr_len/2.25
                # elif 10000 <= fault_len <12000:
                #     corr_len = curr_len/2.75
                # elif 3000 <= fault_len <10000:
                #     corr_len = curr_len/3.25
                # # elif 6000 <= fault_len <12000:
                #     # fault_len = curr_len/8
                # elif 2000 <= fault_len <3000:
                #     corr_len = curr_len/4.5
                    
                # else:
                    
               
                if idx in [115,116,117]:
                    corr_len = curr_len*1.35
                if idx in [124,125,33,34,41,42,46,63,64,74,77,78,79,80,83,84]:
                    corr_len = curr_len/2
                if idx in [105,106,25,26,23,24]:
                    corr_len = curr_len/1.8
                if idx in [122,123]:
                ...
                for i in space:
                    # s = substring(geom.loc[df_id], i, i+steps)
                ...
                min_indx =min_indxs[len(min_indxs)//2][-1]
                ...
                # new_geom = cut(geom[df_id],new_length)[0]
                new_geom = cut(geom,new_length)[0]
                # tran_max_displs = [masked_df.loc[masked_df['tran_id']==str(sub_idx),'displacement'].max() for sub_idx in sub_idxs]
                ...
                # dataframe.loc[(dataframe.id ==1) & (dataframe.TR_SEGMENT==1), 'geometry']=new_geom
                # dataframe.loc[(dataframe.id ==1) & (dataframe.TR_SEGMENT==1), 'Heave(m)']=new_length
                # dataframe.loc[(dataframe.id ==1) & (dataframe.TR_SEGMENT==1), 'Throw']=min_prof[0]-min_prof[-1]
                # geom_mask = dataframe.mask(dataframe.id==idx) | (dataframe.TR_SEGMENT==sub_idx))
            ...
            # print(np.mean(TranLen))
            transect_gdf_upd.loc[mask, 'Tran AvL(m)'] = np.mean(TranLen)
            # transect_gdf_upd.loc[mask,'Tran AvL(m)'] = 10#np.mean(TranLen)
    # transect_gdf_upd.insert(4,'Tran AvL(m)',np.mean(transect_gdf_upd['Tran L(m)']))
...
################################# FOURTH BLOCK
#statistic_df = gpd.read_file('/media/hyradus/I-DATS/Working/Erica-FFC/Humboldt/Processing_original/faults-Humboldtv03_repr_densified_0_complete_statistics.shp')
...
fname = processing_folder+'/Humboldt_MaxDisp_Length_normalized.eps'
...
# ax.set_title("y=$%.6fx^{%.6f}$ \n$R^{2}$: %.6f"%(popt_log[0],popt_log[1],r_squared))
ax.set_title('Humboldt MaxD/L')
fname = processing_folder+'/Humboldt_maxDLength_ratio.eps'
plt.savefig(fname, format='eps')
...
# ax.set_xticks(np.arange(0,50, step=1))
# ax.set_title("y=$%.6fx^{%.6f}$ \n$R^{2}$: %.6f"%(popt_log[0],popt_log[1],r_squared))
ax.set_title('Humboldt Max Displacement/Max Aperture')
fname = processing_folder+'/Humboldt_maxD-maxA log.eps'
plt.savefig(fname, format='eps')
...
# ax.set_xticks(np.arange(0,50, step=1))
# ax.set_title("y=$%.6fx^{%.6f}$ \n$R^{2}$: %.6f"%(popt_log[0],popt_log[1],r_squared))
ax.set_title('Humboldt Length/Aperture')
fname = processing_folder+'/Humboldt_Ape-Len log.eps'
plt.savefig(fname, format='eps')
plt.savefig(fname.split('.eps')[0]+'.png', format='png')
...
for plt_chnk in plot_chunks:
    ...
    id1 = str(plt_chnk[0])
    id2=str(plt_chnk[-1])
    fname = processing_folder+'/Humboldt_Displacement_along_faults_'+"{0}-{1}".format(id1, id2)+'normalized.png'
    
    plt.savefig(fname, dpi=150)
    fname = processing_folder+'/Humboldt_Displacement_along_faults_'+"{0}-{1}".format(id1, id2)+'normalized.pdf'
